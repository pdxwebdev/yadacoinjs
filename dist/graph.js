...
define(["require", "exports"], function (require, exports) {
    ...
    class Graph {
        ...
        _getMail(ridsoverride, collection = this.identity.collections.MAIL) {
            ...
            let rids = ridsoverride || [this.identity.generateRid(this.identity.identity.username_signature, this.identity.identity.username_signature, this.identity.collections.MAIL),
        ...
        prepareMailItems(label) {
            return this.graph.mail.filter((item) => {
                if (label === 'Sent' && item.public_key === this.identity.identity.public_key)
                    return true;
                if (label === 'Inbox' && item.public_key !== this.identity.identity.public_key)
                    return true;
                return false;
            }).map((item) => {
        ...
        _sendMail(params) {
            return __awaiter(this, void 0, void 0, function* () {
                const rid = this.generateRid(this.identity.identity.username_signature, params.recipient.username_signature);
                const requester_rid = this.generateRid(this.identity.identity.username_signature, this.identity.identity.username_signature, params.collection);
                const requested_rid = this.generateRid(params.recipient.username_signature, params.recipient.username_signature, params.collection);
        ...
        addFriend(identity, rid = '', requester_rid = '', requested_rid = '') {
            rid = rid || this.generateRid(this.identity.identity.username_signature, identity.username_signature);
            requester_rid = requester_rid || this.generateRid(this.identity.identity.username_signature, this.identity.identity.username_signature, this.identity.collections.CONTACT);
            requested_rid = requested_rid || this.generateRid(identity.username_signature, identity.username_signature, this.identity.collections.CONTACT);
        ...
        getFriendRequests(rid = null) {
            return new Promise((resolve, reject) => {
                const rids = [rid || this.generateRid(this.identity.identity.username_signature, this.identity.identity.username_signature, this.identity.collections.CONTACT)];
        ...
        _addGroup(identity, rid = '', requester_rid = '', requested_rid = '', refresh = true) {
            ...
            rid = rid || this.generateRid(this.identity.identity.username_signature, identity.username_signature);
            requester_rid = requester_rid || this.generateRid(this.identity.identity.username_signature, this.identity.identity.username_signature, identity.collection);
        ...
        getGroups(rid = null, collectionName = 'group', ignoreCache = false) {
            const root = !rid;
            rid = rid || this.generateRid(this.identity.identity.username_signature, this.identity.identity.username_signature, collectionName);
