...
export class Transaction {
  ...
  async generateTransaction(info: GraphI.TxnParams) {
    const version = 4;
    this.txnattempts = [12, 5, 4];
    this.cbattempts = [12, 5, 4];
    ...
    this.unspent_transaction_override = this.info
      .unspent_transaction as GraphI.Txn;
    ...
    //     }
    // }
    var myAddress = await this.identity.publicKeyToAddress(
    ...
    var outputs_hashes_arr = outputs_hashes.sort(function (
      a: string,
      b: string
    ) {
    ...
    if (this.info.dh_public_key && this.info.relationship.dh_private_key) {
      ...
      this.transaction.relationship = this.crypt.publicEncrypt(
        JSON.stringify(this.info.relationship),
        this.recipient_identity.public_key
      );
      var hash = await this.identity.sha256(
        this.transaction.public_key +
      ...
      this.info.relationship[this.identity.collections.SMART_CONTRACT]
    ) {
      ...
      this.transaction.relationship[
        this.identity.collections.SMART_CONTRACT
      ].creator = this.crypt.shared_encrypt(
        this.info.shared_secret,
        JSON.stringify(
          this.transaction.relationship[
            this.identity.collections.SMART_CONTRACT
      ...
      if (this.info.relationship[this.identity.collections.GROUP].parent) {
        this.transaction.relationship = this.crypt.shared_encrypt(
          this.info.relationship[this.identity.collections.GROUP].parent
            .username_signature,
          JSON.stringify(this.info.relationship)
        );
      } else {
        this.transaction.relationship = this.crypt.encrypt(
      ...
      this.info.relationship[
        this.identity.collections.WEB_CHALLENGE_RESPONSE
      ] ||
      this.info.relationship[this.identity.collections.WEB_PAGE_REQUEST] ||
  ...
  async sendTransaction(txn: any = null, transactionUrlOverride = undefined) {
    const address = await this.identity.publicKeyToAddress(txn.public_key);
