...
export class Graph {
  ...
  parseMail(
    messages: GraphI.TxnParams[],
    collection = this.identity.collections.MAIL
  ) {
    return new Promise((resolve, reject) => {
      var chats = [];
        ...
        if (this.groups_indexed[message.requested_rid as string]) {
          try {
            let identity = this.getIdentityFromTxn(
              this.groups_indexed[message.requested_rid as string],
              this.identity.collections.GROUP
            );
            if (!identity) continue;
            var decrypted = this.crypt.shared_decrypt(
          ...
          if (messageJson[collection]) {
            ...
            try {
              message.relationship[collection] = JSON.parse(
                Base64.decode(messageJson[collection])
              );
            } catch (err) {
    ...
    if (this.isGroup(recipient)) {
      ...
      if (dh_public_key && dh_private_key) {
        var privk = new Uint8Array(
          (dh_private_key.match(/[\da-f]{2}/gi) as RegExpMatchArray).map(
            function (h) {
        ...
        var pubk = new Uint8Array(
          (dh_public_key.match(/[\da-f]{2}/gi) as RegExpMatchArray).map(
            function (h) {
              return parseInt(h, 16);
            }
          )
  ...
  addFriend(
    ...
    var raw_dh_private_key = foobar.bitcoin.crypto.sha256(
      this.identity.identity.key.toWIF() + identity.username_signature
    );
  ...
  generateRid(
    ...
    collection: IdentityI.Collections | string = ""
  ) {
  ...
  generateRids(
    identity: IdentityI.Identity,
    identity2: IdentityI.Identity | null = null,
    collection: IdentityI.Collections | null = null
  ) {
  ...
  isMe(identity: IdentityI.Identity) {
    ...
    return (
      identity.username_signature === this.identity.identity.username_signature
    );
  }
  ...
  sortAlpha(list: Generic.DynamicObjectString[], key: string, reverse = false) {
    list.sort(
      (a: Generic.DynamicObjectString, b: Generic.DynamicObjectString) => {
      ...
      for (let i in this.keys) {
        ...
        for (var j = 0; j < this.keys[i].dh_private_keys.length; j++) {
          ...
          for (var k = 0; k < this.keys[i].dh_public_keys.length; k++) {
            ...
            var privk = new Uint8Array(
              (dh_private_key.match(/[\da-f]{2}/gi) as any).map(function (
                h: any
              ) {
                return parseInt(h, 16);
              })
            ...
            var pubk = new Uint8Array(
              (dh_public_key.match(/[\da-f]{2}/gi) as any).map(function (
                h: any
              ) {
                return parseInt(h, 16);
              })
            );
            var shared_secret = this.crypt.toHex(
              X25519.getSharedKey(privk, pubk)
            );
